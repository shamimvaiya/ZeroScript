...
if connector_registry:
    connector_registry.register(RobloxStudioConnector(mcp_manager=mgr), set_active=True)
    connector_registry.register(VSCodeConnector())
    connector_registry.register(UnityConnector())
    connector_registry.register(AndroidStudioConnector())
clients = set()
...
def safe_call(name, arguments, timeout):
    ...
    if connector_registry:
        if "/" in name:
            ...
            if prefix != "roblox" and connector_registry.get(prefix):
                return connector_registry.call_tool(name, arguments, timeout)
        elif connector_registry.active_id != "roblox":
            active_conn = connector_registry.get_active()
            if active_conn and active_conn.id != "roblox":
                return active_conn.call_tool(name, arguments, timeout)
    ...
    try:
        ...
        async for raw in ws:
            ...
            if mtype == "ping":
                ...
                cid = msg.get("connector_id")
                ok = False
                if connector_registry and cid:
