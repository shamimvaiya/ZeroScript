...
const ZSParse = (() => {
  ...
  const customBlockParsers = [];
  function registerBlockParser(parserFn) {
    if (typeof parserFn === "function") customBlockParsers.push(parserFn);
  }
  ...
  function parseToolCalls(r) {
    ...
    // Additional connector custom block parsers (e.g. C#, Shell, etc.)
    if (out.length === 0 && customBlockParsers.length > 0) {
      for (const parserFn of customBlockParsers) {
        try {
          const res = parserFn(r);
          if (res) {
  ...
  return {
    ...
    isInjectedFeedback, hasCommandShape, registerBlockParser,
  };
})();
